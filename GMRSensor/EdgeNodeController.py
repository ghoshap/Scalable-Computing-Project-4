...
from Component.GMRSensor import GMRSensor
...
class EdgeNodeController (EdgeController) :
    ...
    def Run(self):
        time.sleep(3)
        temp = self.ReadGlucoseLevel()
        print("GlucoseLevel---->",str(temp))
        time.sleep(3)
        if (temp>400 or temp<75):
            self.UartRx(data=temp)
        time.sleep(3)

    def ReadGlucoseLevel(self):
        self.I2CRead()
    ...
    def UartRx(self,**kwargs):
        ...
        if (len(tempArray) == 1) : 
            ...
            data_ = tempArray[0]
            topic_ = tempArray[1]
        encdata = self.Encript(data_)
        self.WifiPowerConsumed(encdata)
        #if(temp>400 or temp<70):
        self._mqttService.Publish(encdata,topic_)