...
from Component.EMGSensor import EMGSensor
...
class EMGController (MicroController) :
    ...
    def Run(self):
        time.sleep(3)
        temp = self.ReadEMGLevel()
        print("EMGLevel---->",str(temp))
        #time.sleep(3)
        if (temp>30 or temp<0.05):
            self.WriteBluetooth(temp)
        time.sleep(3)

    def ReadEMGLevel(self):
        self.I2CRead()
        return self.ts.I2CRead()

    def WriteBluetooth(self,data):
        super().I2CWrite()
        data = str(data) + '| EMGLevel'
        self.ble.Tx(data)